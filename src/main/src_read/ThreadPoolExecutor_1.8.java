...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    // Integer.SIZE 表示 int 类型数据的位数，32位（4个字节）
    // COUNT_BITS = 29
    private static final int COUNT_BITS = Integer.SIZE - 3;

    // 线程最大容量（不是任务）：536870911
    ...
    public void execute(Runnable command) {
        ...
        // 线程数 < 最大线程限制，则创建一个新的线程执行任务（无论有空闲的线程与否）
        ...
        // 线程数达到最大值，判断线程池是否在运行，然后添加任务到队列中
        if (isRunning(c) && workQueue.offer(command)) {
            int recheck = ctl.get();
            /**
             * 1、线程池即将停止，则移除任务，拒绝任务
             * 2、线程池处于 running 状态，但是没有存活的 worker，则创建线程
             */
