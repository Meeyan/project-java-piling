...
public class ReentrantLock implements Lock, java.io.Serializable {
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        final boolean nonfairTryAcquire(int acquires) {
            ...
            /**
             * state:
             *   0 ： (第一次加锁) 表示当前没有被线程加锁，尝试加锁，设置独占模式，加锁完成。
             *   非0 ：有线程在使用
             *      第一种：当前线程和独占线程相同，则加锁次数递增一次（锁重入），加锁完成。
             *      第二种：当前线程和独占线程不同，return false，表示加锁失败
             */
            if (c == 0) {
                // 初次加锁，acquires为1，加锁成功后，当前线程独占，加锁完成。
    ...
    static final class NonfairSync extends Sync {
        ...
        final void lock() {
            ...
            /**
             * compareAndSetState(0, 1)：表示第一次加锁，如果成功，则设置当前线程为独占
             * acquire(1)：表示锁已经被使用过（加锁过）,尝试从新加锁
             */
