...
public class StampedLock implements java.io.Serializable {
     ...
     * updates.
     *
     * Waiters use a modified form of CLH lock（自旋锁） used in
    ...
    private static final long WBIT = 1L << LG_READERS;  // = 128，最大可等待队列数？
    private static final long RBITS = WBIT - 1L;    // R=read 最大的可用读的值（the max reader count value），即127
    private static final long RFULL = RBITS - 1L;   // 126
    private static final long ABITS = RBITS | WBIT;     // = 255, | 按位或运算
    ...
    static final class WNode {
        volatile WNode prev;    // 前一个
        volatile WNode next     // 下一个
    ...
    /**
     * Head of CLH queue 自旋队列头元素
     */
    private transient volatile WNode whead;
    /**
     * Tail (last) of CLH queue 自旋队列尾元素
     */
    ...
    public long readLock() {
        ...
        /*
         * s的初始状态值是256（即：0000 0000 0000 0000 0000 0001 0000 0000），即stamp从256开始
         *   whead == wtail : 表示队列中没有等待的线程
         *   s & ABITS : s 为当前的stamp，ABITS=255（0000 1111 1111），RFULL=126，所以s & ABITS = 0
         * 1. ((whead == wtail && (s & ABITS) < RFULL && U.compareAndSwapLong(this, STATE, s, next = s + RUNIT))：仅有一个线程请求锁，且读锁队列尚未满，则尝试加锁（state+1）
         *    如果成功，返回新的state（stamp），如果失败（多个线程或读锁队列已满或cas操作失败）则尝试acquireRead()
         * 2.
         */
    ...
    public void unlockRead(long stamp) {
        ...
        for (; ; ) {
             ...
             * SBITS = -128
             * ABITS = 255（1111 1111）
             *  1. (s = state) & SBITS) != (stamp & SBITS)  : 判断传入的stamp与对象上的偏移量是否相同，不同就异常。
             *  2. (stamp & ABITS) == 0L ：有两种情况stamp=0【失败状态】或者stamp为2的n次方（n>=8）【不大可能】
             *  3. (m = s & ABITS) == 0L ：同上
             *  4. m == WBIT ：已经达到最大（队列）值
             *
             */
    ...
    private void release(WNode h) {
        if (h != null) {
            WNode q;
            Thread w;
            U.compareAndSwapInt(h, WSTATUS, WAITING, 0); // 将字段WSTATUS从等待状态（-1）改为 读模式（0）
            if ((q = h.next) == null || q.status == CANCELLED) {
                // 注意t=t.prev
                for (WNode t = wtail; t != null && t != h; t = t.prev)
                    if (t.status <= 0) // waiting=-1，rmode=0
                        q = t;
            }
    ...
    private long acquireRead(boolean interruptible, long deadline) {
        ...
        for (int spins = -1; ; ) {
            ...
            if ((h = whead) == (p = wtail)) {
                ...
                for (long m, s, ns; ; ) {
                    /*
                     * (m = (s = state) & ABITS) ：当前读锁的队列的等待数量，即m为队列readers的数量
                     *   1. 满足要求，cas自旋更新锁状态
                     *   2.
                     */
